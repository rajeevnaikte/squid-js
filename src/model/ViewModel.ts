...
export class ViewModel
{
  ...
  constructor (viewState: ViewState)
  {
	...
	addItem (view: ViewState | ViewModel, opts?: { itemFor?: string; position?: number }): ViewModel {
		...
		(view as ViewModel).attachTo(this, opts);
...
export class GenesisViewModel {
	...
	add (view: ViewState | ViewModel): ViewModel {
		...
		(view as ViewModel).detach();
		this._domEl.appendChild((view as ViewModel).domEl);
		// eslint-disable-next-line @typescript-eslint/ban-ts-comment
		// @ts-ignore
		if ((view as ViewModel)._style) {
			// eslint-disable-next-line @typescript-eslint/ban-ts-comment
			// @ts-ignore
			document.head.prepend(view._style);
		}
		((view as ViewModel).domEl as CustomElement).postRender?.();
